...
\definecolor{CodeBg}{RGB}{252, 253, 255}
\definecolor{CodeBorder}{RGB}{198, 208, 230}
...
\definecolor{FirstPageBg}{RGB}{246, 238, 255}
\definecolor{FirstPageBand}{RGB}{224, 206, 245}
\definecolor{TitlePageBand}{RGB}{95, 62, 138}
...
\begin{tikzpicture}[remember picture, overlay]
  \fill[FirstPageBg] (current page.south west) rectangle (current page.north east);
  \fill[FirstPageBand] (current page.north west) rectangle ([yshift=-2.8cm]current page.north east);
\end{tikzpicture}
\vspace*{4cm}
\begin{center}
  {\Huge\bfseries\color{NavyBlue} AI Inference Engineering}\\[0.6em]
  {\Large\color{AccentOrange} Building Fast, Scalable, and\\[0.2em]
   Production-Ready AI Systems}\\[1.2em]
  {\large\color{DarkGray} Author: Shivam Kumar}
\end{center}
\clearpage
...
\thispagestyle{empty}
\begin{tikzpicture}[remember picture, overlay]
  \fill[TitlePageBand] (current page.north west) rectangle
...
\begin{center}
  ...
  {\normalsize\color{DarkGray} Author: Shivam Kumar \\ Website: \url{https://shivam2003.com}}\\[1em]
...
\noindent Author: Shivam Kumar\\
\noindent Website: \url{https://shivam2003.com}\\[0.5em]
...
\begin{longtable}{@{}p{3.8cm}p{4.8cm}p{4.5cm}@{}}
  \toprule
  \textbf{Task} & \textbf{API} & \textbf{Notes} \\
  \midrule
  \endfirsthead
  \toprule
  \textbf{Task} & \textbf{API} & \textbf{Notes} \\
  \midrule
  \endhead
  Allocate device memory & \code{cudaMalloc}, \code{cudaMallocAsync} & Use async variants with streams to reduce stalls. \\
  Free device memory & \code{cudaFree}, \code{cudaFreeAsync} & Always free in reverse ownership order to simplify debugging. \\
  Host/device transfer & \code{cudaMemcpy}, \code{cudaMemcpyAsync} & Minimize transfers; keep tensors on GPU between kernels. \\
  Launch kernels & \code{kernel<<<grid, block, shmem, stream>>>} & Start with 256 threads/block for most inference kernels. \\
  Thread indexing & \code{blockIdx}, \code{threadIdx}, \code{blockDim} & Global index in 1D: \code{idx = blockIdx.x * blockDim.x + threadIdx.x}. \\
  Synchronization & \code{cudaDeviceSynchronize}, \code{cudaStreamSynchronize}, \code{\_\_syncthreads()} & Prefer stream sync over device-wide sync in production. \\
  Error handling & \code{cudaGetLastError}, \code{cudaGetErrorString} & Wrap all CUDA calls in a check macro. \\
  Device query & \code{cudaGetDeviceProperties} & Read SM count, shared memory, max threads/block before tuning. \\
  \bottomrule
\end{longtable}

\section*{Minimal Kernel Checklist}
\begin{itemize}
  \item Validate bounds: every kernel should guard out-of-range threads.
  \item Use contiguous memory access patterns for coalescing.
  \item Reuse data in shared memory when read multiple times.
  \item Avoid warp divergence in hot paths.
  \item Profile before optimizing; trust metrics, not intuition.
\end{itemize}
...
\section*{Essential Linux Command Map}

\begin{longtable}{@{}p{3.6cm}p{5.6cm}p{3.9cm}@{}}
  \toprule
  \textbf{Goal} & \textbf{Recommended Command} & \textbf{Why It Matters} \\
  \midrule
  \endfirsthead
  \toprule
  \textbf{Goal} & \textbf{Recommended Command} & \textbf{Why It Matters} \\
  \midrule
  \endhead
  Check GPU health & \code{nvidia-smi} & Fast snapshot of util, memory, thermals. \\
  Watch GPU over time & \code{nvidia-smi dmon -s u} or \code{nvtop} & Detect bursts and stalls that averages hide. \\
  Inspect model disk size & \code{du -sh /models/*} & Prevent slowdowns from disk pressure. \\
  Check free memory & \code{free -h} & Spot host-memory pressure before OOM. \\
  Verify API port & \code{ss -tlnp | grep 8000} & Confirms serving process is bound and reachable. \\
  Validate health endpoint & \code{curl http://localhost:8000/health} & Quick liveness check for probes and load balancers. \\
  List loaded models & \code{curl http://localhost:8000/v1/models} & Verifies runtime model registration. \\
  Benchmark quickly & \code{time python inference\_benchmark.py} & Baseline latency and throughput before tuning. \\
  Control visible GPUs & \code{CUDA\_VISIBLE\_DEVICES=0,1} & Isolate workloads and avoid noisy neighbors. \\
  Keep jobs alive & \code{tmux new -s inference} & Run long jobs safely over unstable sessions. \\
  \bottomrule
\end{longtable}
...
\section*{Benchmark Design Template}

\begin{longtable}{@{}p{3.8cm}p{3.5cm}p{5.8cm}@{}}
  \toprule
  \textbf{Category} & \textbf{Field} & \textbf{Recommended Default} \\
  \midrule
  \endfirsthead
  \toprule
  \textbf{Category} & \textbf{Field} & \textbf{Recommended Default} \\
  \midrule
  \endhead
  Endpoint & URL & \code{http://localhost:8000/v1/chat/completions} \\
...
\section*{Runbook}
\begin{enumerate}
  \item Warm up the runtime and model weights.
  \item Execute fixed-shape tests (same prompt and output length).
  \item Sweep concurrency and collect percentile metrics.
  \item Repeat each configuration at least 3 times.
  \item Report median of runs and attach p95/p99 tails.
\end{enumerate}
...
\begin{longtable}{@{}p{1.1cm}p{3.2cm}p{4.8cm}p{4.2cm}@{}}
  \toprule
  \textbf{Step} & \textbf{Focus} & \textbf{Primary Tool / Command} & \textbf{What to Look For} \\
  \midrule
  \endfirsthead
  \toprule
  \textbf{Step} & \textbf{Focus} & \textbf{Primary Tool / Command} & \textbf{What to Look For} \\
  \midrule
  \endhead
  1 & Health snapshot & \code{nvidia-smi} & GPU utilization, memory usage, temperature, and power draw. \\
  2 & Timeline bottlenecks & \code{nsys profile --trace=cuda,nvtx,osrt,cudnn,cublas --sample=cpu --output=my\_profile python my\_inference.py} & Idle gaps between kernels, heavy memcpy phases, weak CPU/GPU overlap. \\
  3 & Kernel deep dive & \code{ncu --set full --kernel-name cutlass --launch-skip 10 --launch-count 3 -o kernel\_analysis python run\_matmul.py} & SM active cycles, memory throughput, L2 hit rate, warp stalls. \\
  ...
  5 & KV cache behavior & \code{curl http://localhost:8000/metrics | grep cache} & \code{vllm:gpu\_cache\_usage\_perc} below 90\%, hit rate trending upward. \\
  \bottomrule
\end{longtable}

\section*{Profiling Interpretation Playbook}

\begin{itemize}
  \item If GPU utilization is low and queue depth is low: increase effective batch size.
  \item If TTFT spikes but TPOT is stable: investigate prefill pressure and scheduler fairness.
  \item If TPOT rises with long prompts: decode is likely memory-bound; test quantization or shorter max output.
  \item If memory is consistently above 95\%: reduce sequence length, lower max concurrent sequences, or add GPUs.
  \item If throughput is flat while latency worsens: you are saturation-limited; scale horizontally.
\end{itemize}

\section*{MFU Reference}

\[
\text{MFU} = \frac{\text{achieved FLOP/s}}{\text{peak FLOP/s}}
\]

\noindent Practical rule: for inference, MFU above 30\% is generally healthy for memory-heavy decode; compute-heavy prefill can exceed 50\% when tuned.
