...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[LE]{\small\color{gray}\textit{Chapter 1 · Introduction to AI Inference Engineering}}
...
\titleformat{\chapter}[display]
  ...
  {16pt}
  {\Huge\bfseries}
  [\vspace{4pt}\color{chaptercolor}\titlerule[2pt]\vspace{2pt}
   \color{chaptercolor!40}\titlerule[0.5pt]]
...
{\fontsize{13}{16}\selectfont\MakeUppercase{\textls[150]{AI Inference Engineering}}}
...
\vspace{2.5cm}
\color{white!60}
{\small\itshape First Edition · 2025}
...
\begin{tikzpicture}[scale=1.0]
  ...
  % Small icons
  \node[font=\Large] at (0.2, 0.5) {⚡};
  \node[font=\Large] at (5.8, 0.5) {🚀};
  \node[font=\Large] at (3, 4.6)   {💰};
...
\begin{enumerate}
  \item User query → embedding model inference (fast, $<$10ms)
  \item Embedding → vector database search
  \item Retrieved documents + query → LLM inference (slower, 1--10s)
\end{enumerate}
...
def run_inference(prompt: str, max_new_tokens: int = 100) -> dict:
    ...
    # Convert raw text → token IDs → tensor on GPU
